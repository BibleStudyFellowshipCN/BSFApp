...
export function loadLesson(lesson) {
  return async(dispatch) => {
    try {
      ...
      const lessonContent = await loadAsync(Models.Lesson, `${lesson.id}.json`);
      if (lessonContent) {
        // Now that we received the json, we dispatch an action to update the stage
        dispatch({
          type: RECEIVE_LESSON,
          payload: {lesson: lessonContent},
        })
      }
    } catch(error) {
      console.log(error)
...
const ACTION_HANDLERS = {
  [RECEIVE_LESSON]: (state, action) => Object.assign({}, state, {[action.payload.lesson.id]: action.payload.lesson}),
}
